...
def test_add_task_creates_list_and_calc(todo_file):
    new_id = store.add_task(todo_file, "beta", "write docs")
...
def test_set_task_status_recomputes_calc(todo_file):
    store.add_task(todo_file, "beta", "t1")   # id 1
    store.add_task(todo_file, "beta", "t2")   # id 2
    assert store.set_task_status(todo_file, "beta", 1, "done") is True
    it = store.resolve_item(todo_file, "beta")
    assert it["tasks"][0]["status"] == "done"
    # one done, one todo → highest present is todo
    assert it["calc_status"] == "todo"
    store.set_task_status(todo_file, "beta", 2, "in-progress")
    it = store.resolve_item(todo_file, "beta")
...
def test_all_done_rolls_calc_to_done(todo_file):
    store.add_task(todo_file, "beta", "t1")   # id 1
    store.set_task_status(todo_file, "beta", 1, "done")
    it = store.resolve_item(todo_file, "beta")
...
def test_remove_last_task_drops_calc_status(todo_file):
    store.add_task(todo_file, "beta", "t1")   # id 1
    assert store.remove_task(todo_file, "beta", 1) is True
    it = store.resolve_item(todo_file, "beta")
    assert it["tasks"] == []
...
def test_mutations_on_missing_item_return_false(todo_file):
    assert store.add_task(todo_file, "nope", "x") is None
    assert store.set_task_status(todo_file, "nope", 1, "done") is False
    assert store.remove_task(todo_file, "nope", 1) is False


def test_add_task_with_phase_stores_and_emits_it(todo_file):
    store.add_task(todo_file, "beta", "scaffold", phase=2)   # id 1
    it = store.resolve_item(todo_file, "beta")
...
def test_set_task_phase_resorts_and_can_clear(todo_file):
    store.add_task(todo_file, "beta", "x")                 # id 1, unphased
    store.add_task(todo_file, "beta", "y", phase=1)        # id 2, sorts before x
    ...
    store.set_task_phase(todo_file, "beta", 1, None)
    it = store.resolve_item(todo_file, "beta")
